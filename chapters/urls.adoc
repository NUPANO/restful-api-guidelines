...
* [[q]]{q}: default query parameter, used for full-text search across (potentially) multiple object attributes.
Returns results that only match partially.
The API documentation {MUST} specify which attributes are included in the search.
* [[sort]]{sort}:
...
* Pagination: see <<160>> for paginated related parameters
